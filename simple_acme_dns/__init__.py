...
class ACMEClient:
    ...
    def __init__(
            self, csr, domains=None, email=None, directory=None, nameservers=None, new_account=False
    ):
        """
        ...
        # Automatically create a new private key and CSR
            ...
            self.csr = csr
            self.domains = []
            for attribute in x509.load_pem_x509_csr(csr).subject.get_attributes_for_oid(x509.NameOID.COMMON_NAME):
                self.domains.append(attribute.value)


#            decoded_cert = x509.load_pem_x509_csr(csr)
