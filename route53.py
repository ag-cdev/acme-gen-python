...
class DNS():
    ...
    def _change_cname_record(self, action, subdomain, cname, domain_name):
        zone_id = self._find_zone_id_for_domain(domain_name)
        domain_dns_value = subdomain + "." + domain_name
        cname_dns_value = cname + "." + domain_name
        rrecords = self._resource_records[domain_name]
        record_value = {"Value": '"{0}"'.format(cname_dns_value)}
        if action == "DELETE":
            # Remove the record being deleted from the list of tracked records
            rrecords.remove(record_value)
            if rrecords:
                # Need to update instead, as we're not deleting the rrset
                action = "UPSERT"
            else:
                # Create a new list containing the record to use with DELETE
                rrecords = [record_value]
        else:
            rrecords.append(record_value)

        response = self.r53.change_resource_record_sets(
            HostedZoneId=zone_id,
            ChangeBatch={
                "Comment": "cname record - " + action,
                "Changes": [
                    {
                        "Action": action,
                        "ResourceRecordSet": {
                            "Name": domain_dns_value,
                            "Type": "CNAME",
                            "TTL": self.ttl,
                            "ResourceRecords": rrecords,
                        },
